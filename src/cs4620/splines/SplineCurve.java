...
public abstract class SplineCurve {
	...
	private void setBeziers() {
		...
		for(int i = 1; i < controlPoints.size()-2; ++i) {
			this.bezierCurves.add(new CubicBezier(controlPoints.get(i-1), controlPoints.get(i),
		...
		if(isClosed) {
			...
			this.bezierCurves.add(new CubicBezier(controlPoints.get(lastIndex-2), controlPoints.get(lastIndex-1),
					controlPoints.get(lastIndex), controlPoints.get(0), epsilon));
			this.bezierCurves.add(new CubicBezier(controlPoints.get(lastIndex-1), controlPoints.get(lastIndex-2),
					controlPoints.get(0), controlPoints.get(1), epsilon));
			this.bezierCurves.add(new CubicBezier(controlPoints.get(lastIndex), controlPoints.get(0),
